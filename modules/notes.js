...
function initNotes() {
  ...
  if (!container.dataset.delegated) {
    container.addEventListener('click', function(e) {
      ...
      if (card) {
        notesDownload(card.dataset.noteId, card.src);
        return;
      }
    });
...
function showNotesListHeader() {
  ...
  right.innerHTML = '<button class="icon-btn" aria-label="Удалить все" id="notesDeleteAllBtn">'
    + window.ICONS['trash-2'] + '</button>';
  var deleteAllBtn = document.getElementById('notesDeleteAllBtn');
  if (deleteAllBtn) {
    deleteAllBtn.onclick = function() { notesDeleteAll(); };
  }
}
...
function showNotesDrawHeader() {
  ...
  left.innerHTML = '<button class="icon-btn notes-text-btn" aria-label="Отмена">Отмена</button>';
  left.onclick = function() { showNotesList(); };

  center.innerHTML = '<div class="hc-default">Новая заметка</div>';

  right.innerHTML = '<button class="icon-btn notes-text-btn" aria-label="Сохранить">Сохранить</button>';
  ...
  notesLoadAllDB().then(function(notes) {
    ...
    if (notes.length === 0) {
      window.app.hideSkeleton(container,
        ...
        '</div>' +
        '<button class="notes-fab" aria-label="Новая заметка">' +
    ...
    for (var i = 0; i < notes.length; i++) {
      ...
      html += '<div class="notes-card" data-note-id="' + note.id + '">' +
        ...
        '<button class="notes-card-delete" data-note-id="' + note.id + '" ' +
        'aria-label="Удалить заметку">' + window.ICONS['trash-2'] + '</button>' +
        '</div>' +
        '</div>';
    }
    html += '</div>';
    html += '<button class="notes-fab" aria-label="Новая заметка">' +
      window.ICONS.plus + '</button>';

    window.app.hideSkeleton(container, html);
  }).catch(function() {
    window.app.showError(container, 'Не удалось загрузить заметки', showNotesList);
  });
}
...
function showNotesDrawView() {
  ...
  container.innerHTML =
    ...
    '<button class="notes-tool-eraser" aria-label="Ластик"></button>' +
    '</div>' +
    '</div>' +
...
function initNotesCanvas() {
  ...
  var headerH  = parseInt(getComputedStyle(document.documentElement)
    .getPropertyValue('--header-height'), 10) || 56;
  var toolbarH = 52;
  var safeTop  = window.visualViewport
