...
function renderItems(block) {
  return block.items.map(function(item) {
    ...
    var noteHtml = item.note ? '<span class="cl-item-note">' + formatNote(item.note) + '</span>' : '';
    return '<label class="cl-item' + (isRequired ? '' : ' cl-item--optional') + '">' +
      ...
      (isRequired ? '' : '<span class="cl-optional-badge">необязательно</span>') +
      noteHtml +
      '</span></label>';
  }).join('');
}
...
function renderBlock(block) {
  var blockNoteHtml = block.note ? '<div class="cl-block-note">' + formatNote(block.note) + '</div>' : '';
  return '<div class="cl-block" data-block-id="' + block.id + '">' +
    ...
    '</div>' +
    blockNoteHtml +
    '<div class="cl-block-content" data-content="' + block.id + '">' +
    renderItems(block) +
    '</div></div>';
}

function updateBadge(blockId) {
  var blockEl = document.querySelector('.cl-block[data-block-id="' + blockId + '"]');
  if (!blockEl) return;
  ...
  if (allChecked) { badge.classList.add('ok'); blockEl.classList.add('block-complete'); }
  else { badge.classList.remove('ok'); blockEl.classList.remove('block-complete'); }
}
...
function restoreCheckboxState(container) {
  ...
  var blocks = container.querySelectorAll('.cl-block');
...
function renderAllBlocks(data) {
  ...
  for (var i = 0; i < blocks.length; i++) {
    var block = blocks[i];
    if (block.type === 'divider') html += renderDivider(block);
    else html += renderBlock(block);
  }
  return html;
}
...
function renderChecklistsHeader() {
  ...
  left.innerHTML = '<button class="icon-btn" aria-label="Назад" onclick="window.app.closeBottomPanel(); window.app.navigateTo(\'main\')">' + window.ICONS.back + '</button>';
  center.innerHTML = '<div class="cl-tab-switcher" id="checklistTabGroup">' +
    '<button class="cl-tab-pill' + (activeTab === 'safa' ? ' cl-tab-pill--active' : '') + '" data-tab="safa">SAFA</button>' +
    '<button class="cl-tab-pill' + (activeTab === 'customs' ? ' cl-tab-pill--active' : '') + '" data-tab="customs">Customs</button>' +
    '</div>';
  if (!center.dataset.tabDelegated) {
    center.addEventListener('click', function(e) {
      var btn = e.target.closest('[class*="cl-tab-pill"]');
      if (!btn) return;
      var group = center.querySelector('.cl-tab-switcher');
      if (group) {
        group.querySelectorAll('.cl-tab-pill').forEach(function(b) { b.classList.remove('active'); });
      }
...
function initChecklists() {
  ...
  if (!container.dataset.delegated) {
    container.addEventListener('click', function(e) {
      var header = e.target.closest('.cl-block-header');
      if (header) { var block = header.closest('.cl-block'); if (block) block.classList.toggle('open'); return; }
    });
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initCommitsPanelListeners);
else initCommitsPanelListeners();