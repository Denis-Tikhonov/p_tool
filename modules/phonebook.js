...
function getPhoneBadgeClass(type) { return type === 'work' ? 'phone-badge--work' : 'phone-badge--personal'; }
function getPhoneBadgeText(type) { return type === 'work' ? 'раб.' : 'личн.'; }
...
function matchesSearch(contact, query) {
  ...
  if (contact.name.toLowerCase().indexOf(lq) !== -1) return true;
...
function initPhonebook() {
  ...
  // container не требует dataset.delegated — нативные ссылки
